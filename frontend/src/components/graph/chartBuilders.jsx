              ...
              <div className="bg-[#0b0f14] border border-primary rounded-lg px-3 py-2 shadow-lg">
                ...
                <div className="text-sm text-gray-200">
                  ...
                  <span className="font-semibold">
                    {Number(roic).toFixed(2)}%
                  </span>
                </div>
                <div className="text-sm text-gray-200">
                  EV / EBIT:{" "}
                  <span className="font-semibold">
                    {Number(evEbit).toFixed(2)}
                  </span>
                </div>
              </div>
            );
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      ...
      >
        ...
        />
        <Tooltip
          labelFormatter={(l) => `Data: ${tooltipDate(l)}`}
          formatter={(v, name, p) => [
            v?.toFixed?.(2) ?? v,
            `P/VP (${p?.payload?.symbol})`,
          ]}
        />
        <Area
          dataKey="pvp"
    ...
    <ResponsiveContainer width="100%" height={300}>
      ...
      >
        ...
        <Tooltip
          formatter={(v, name, p) => [
            Number(v).toFixed(3),
            `Giro (${p?.payload?.symbol})`,
          ]}
          labelFormatter={(l) => tooltipDate(l)}
        />
        <Line
          type="monotone"
          dataKey="giroAtivos"
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      ...
      >
        ...
        <YAxis
          tickFormatter={(v) =>
            v === null || v === undefined ? "" : `${(v / 1e9).toFixed(1)} B`
          }
          tick={{ fill: "#e5e7eb" }}
          label={{
            value: "Caixa (R$)",
  ...
  return (
    <ResponsiveContainer width="100%" height={350}>
      ...
      >
        ...
        />
        <YAxis
          tickFormatter={(v) => (v ? `${(v / 1e9).toFixed(1)} B` : "")}
          tick={{ fill: "#e5e7eb" }}
        />
        <Tooltip
          formatter={(v, name, p) => [
            Intl.NumberFormat("pt-BR").format(v),
            name && `${name} (${p?.payload?.symbol})`,
          ]}
          labelFormatter={(l) => tooltipDate(l)}
        />
...
export function RoicTimeSeries({ data }) {
  ...
  const prepared = data
    .map((d) => {
      const dateRaw =
        d.dataBalanco || d.data || d.dataDemonstrativoResultados || d.date;
      const ts = parseToTimestamp(dateRaw);
      const roicVal = parseNumberBR(d.roic);
      return {
        ...d,
        __date: ts,
        roic: Number.isFinite(roicVal) ? roicVal : null, // manté null
      };
    })
    .filter((d) => Number.isFinite(d.__date))
    .sort((a, b) => a.__date - b.__date);

  if (prepared.length === 0) return <Empty />;

  // Descomente se quiser checar rapidamente
