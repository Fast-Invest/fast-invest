...
export default function Paginator({
  ...
  const totalPages = useMemo(() => {
    return Math.ceil(
      (cotacoesFiltradas?.length || 0) / (quotationsPerPage || 1)
    );
  }, [cotacoesFiltradas, quotationsPerPage]);

  useEffect(() => {
    if (!cotacoesFiltradas || cotacoesFiltradas.length === 0) {
      setCotacoesVisiveis([]);
      return;
    }

    const totalPagesCalc = Math.ceil(
      cotacoesFiltradas.length / quotationsPerPage
    );
