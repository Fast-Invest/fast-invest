...
export default function ContentDividends({ historico_indicadores = [], dividends = {} }) {
  ...
  const priceFromDividends = typeof dividends.precoAtual === "number" ? dividends.precoAtual : undefined;
  const priceFromHistorico = Array.isArray(historico_indicadores) && historico_indicadores[0] && typeof historico_indicadores[0].precoAtual === "number"
  ...
  const ultimoDiv = last ? Number(last.rate ?? 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "0,00";
  const dataUlt = last ? new Date(last.paymentDate ?? last.lastDatePrior).toLocaleDateString() : "--/--/----";
  ...
  const dyUlt =
    dividends?.dyUlt !== undefined && dividends?.dyUlt !== null
      ...
      : (precoAtual && last ? ((Number(last.rate ?? 0) / precoAtual) * 100).toFixed(2) : "0.00");
  ...
  const totalAno =
    dividends?.totalAno !== undefined && dividends?.totalAno !== null
      ? Number(dividends.totalAno).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })
      : (list
        .filter((it) => {
  ...
  return (
    <div id="dividendos" className="w-full text-white mt-16 p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-10">
        ...
        <div className="p-5 border border-gray-700 rounded-xl">
          <p className="text-gray-400 text-sm">Dividendos {new Date().getFullYear()}</p>
      ...
      <div className="mt-10 border border-gray-700 rounded-xl p-6">
        ...
        <p className="text-gray-400 text-sm mb-6">Últimos dividendos e JCP pagos</p>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {Array.isArray(dividends) && dividends.length > 0 ? (
                dividends.map((item) => (
                  <tr key={item.id ?? `${item.index}-${item.paymentDate}`} className="border-b border-gray-800">
                    <td className="py-3">{item.label ?? item.relatedTo ?? "—"}</td>
                    <td className="py-3">{item.lastDatePrior ? new Date(item.lastDatePrior).toLocaleDateString() : "—"}</td>
                    <td className="py-3">{item.paymentDate ? new Date(item.paymentDate).toLocaleDateString() : "—"}</td>
                    <td className="py-3">R$ {typeof item.rate === "number" ? item.rate.toFixed(2) : (item.rate ?? "0.00")}</td>
                  </tr>
                ))
              ) : (
                <tr>
                  <td colSpan={4} className="py-4 text-center text-gray-500">Nenhum registro encontrado</td>
                </tr>
              )}
            </tbody>
          </table>
