...
export default function QuotationDetails()
{
    ...
    useEffect(() => {
            ...
            async function getData(ticker) 
            {
                ...
                } catch (error){ console.log("erro: ",error);setMostRecentData(null) }
                    
                try {
                    const resp = await quotationService.buscarDetalhesCotacaoAlltime(ticker)
                    setHistoricData(resp?.dados_acao || null)
                } catch (error) {console.log("erro: ", error);setHistoricData(null)}
                
                try {
                    const resp = await quotationService.buscarDividendosCotacao(ticker)
                    setDividendosPagos(resp?.dividendos || null)
                } catch (error) { console.log("erro: ",error);setDividendosPagos(null)} 
            }
    ...
    if(mostRecentData)
    {
        return(
                ...
                <div className="flex bg-bg min-h-screen text-text">
                    ...
                    <NavBar/>
                    <ContentDetails cotacao={mostRecentData} historico_indicadores={historicData} dividendos={dividendosPagos}/>
                    <Footer />
                    </div>
                </div>
                </>
